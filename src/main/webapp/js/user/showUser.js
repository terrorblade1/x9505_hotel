layui.use(['layer','table'], function() {
    ...
    table.render({  //每一次查询都重新渲染table容器
        ...
        ,cols: [[ //表头  field: 'id'表示从实体对象的属性中取到数据放入容器里
            ...
            ,{field: 'username', title: '用户名', align:'center',width:110}
            ,{field: 'pwd', title: '密码', width:320, align:'center',sort: true}
            ,{field: 'roleId', title: '角色', width:120,align:'center',templet:'#roleIdTpl'}
            ,{field: 'firstAuthorityNames', title: '权限', width: 620,align:'center', sort: true,templet:'#statusTpl'}
            ,{field: 'createDate', title: '创建时间', width: 200, align:'center',sort: true}
            ,{field: 'useStatus', title: '是否可用', width: 120, align:'center',sort: true,templet:'#useStatusTpl'}
    ...
    table.on('tool(test)', function(obj) {
        ...
        if (layEvent === 'query'){
            //查看此角色拥有的权限结构
            //1.查询数据并渲染到树型结构容器中
            loadZtree('authority/loadAuthoritiesByRoleId?roleId='+data.roleId);  //访问路径
            //2.弹框显示树型结构容器
            layer.open({
                type:1,  //弹框类型
    ...
    function loadZtree(dataUrl) {
        var setting = {
            ...
            check: {
                enable: false   //是否使用节点复选框，默认为false(不使用)
            },
            async : {
